                ...
                VStack(spacing: 0) {
                    ...
                    if isActive {
                        Button { showingFinishSheet = true } label: {
                            Text("回収する").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 56).background(Color.green).cornerRadius(.buttonCornerRadius)
                        }
                        .padding(.horizontal, 20).padding(.vertical, 16).background(Color(.systemBackground))
                    } else {
                        Button { showingStartSheet = true } label: {
                            Text("水を設置する").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 56).background(Color.appMain).cornerRadius(.buttonCornerRadius)
                        }
                        .padding(.horizontal, 20).padding(.vertical, 16).background(Color(.systemBackground))
                    }
                }
        ...
        NavigationStack {
            ZStack(alignment: .bottom) {
                ...
                VStack(spacing: 0) {
                    ScrollView {
                        VStack(alignment: .leading, spacing: 20) {
                            Text("新規設置").font(.subheadline).foregroundStyle(.secondary).padding(.leading, 4)
                            VStack(spacing: 16) {
                                TextField("設置する水の重さ (g)", text: $startWeight)
                                    .keyboardType(.decimalPad).focused($isInputFocused).monospacedDigit().customTextFieldStyle()
                                TextField("メモ (50文字以内)", text: $note)
                            ...
                            Text("日時").font(.subheadline).foregroundStyle(.secondary).padding(.leading, 4)
                            VStack {
                                DatePicker("記録日時", selection: $recordDate, displayedComponents: [.date, .hourAndMinute])
                ...
                Button { saveStart() } label: {
                    Text("設置を開始する").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 56)
            ...
            .navigationTitle("水を設置する").navigationBarTitleDisplayMode(.inline)
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button { dismiss() } label: { Image(systemName: "xmark") } } }
            .alert("入力エラー", isPresented: $showingValidationAlert) { Button("OK", role: .cancel) { } } message: { Text(validationMessage) }
        }
        ...
        NavigationStack {
            ZStack(alignment: .bottom) {
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 24) {
                            VStack(alignment: .leading, spacing: 12) {
                                Text("回収").font(.subheadline).foregroundStyle(.secondary).padding(.leading, 4)
                                VStack(spacing: 16) {
                                    TextField("回収時の重さ (g)", text: $endWeight)
                                        .keyboardType(.decimalPad).focused($isInputFocused).monospacedDigit().customTextFieldStyle()
                                    TextField("メモ (50文字以内)", text: $note)
                                        .focused($isInputFocused).customTextFieldStyle()
                                        .onChange(of: note) { _, newValue in if newValue.count > 50 { note = String(newValue.prefix(50)) } }
                            ...
                            VStack(alignment: .leading, spacing: 12) {
                                Text("次回の記録").font(.subheadline).foregroundStyle(.secondary).padding(.leading, 4)
                                VStack(spacing: 16) {
                                    TextField("新しい水の重さ (任意)", text: $nextStartWeight)
                                        .keyboardType(.decimalPad).focused($isInputFocused).monospacedDigit().customTextFieldStyle()
                                }
                                .padding().background(Color.white).cornerRadius(16)
                            }
                            VStack(alignment: .leading, spacing: 12) {
                                Text("環境").font(.subheadline).foregroundStyle(.secondary).padding(.leading, 4)
                                VStack(alignment: .leading, spacing: 16) {
                                    HStack {
                                        Text("天気").font(.subheadline).foregroundColor(.secondary)
                                        Spacer()
                                        if selectedWeather != nil { Button("クリア") { selectedWeather = nil }.font(.caption).foregroundColor(Color.appMain) }
                                    }
                                    HStack(spacing: 12) {
                                    ...
                                    Divider()
                                    HStack {
                                        Text("室温").font(.subheadline).foregroundColor(.secondary)
                                        Spacer()
                                        TextField("室温 (任意)", text: $temperature)
                ...
                Button { saveFinish() } label: {
                    Text("記録を更新する").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 56)
            ...
            .navigationTitle("回収する").navigationBarTitleDisplayMode(.inline)
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button { dismiss() } label: { Image(systemName: "xmark") } } }
            .alert("入力エラー", isPresented: $showingValidationAlert) { Button("OK", role: .cancel) { } } message: { Text(validationMessage) }
        }
        .keyboardToolbar(focus: $isInputFocused)
...
struct EditContainerSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .bottom) {
                ...
                ScrollView {
                    VStack(spacing: 0) {
                        Form {
                            Section {
                                TextField("器の名前", text: $editedName).focused($isInputFocused)
                                TextField("空重量 (g) - 任意", text: $editedEmptyWeight).keyboardType(.decimalPad).focused($isInputFocused).monospacedDigit()
                            } header: { Text("器の情報を編集") }
                        }
                        .frame(height: 240)
                        Button(role: .destructive) { showingDeleteConfirmation = true } label: { Text("この器を削除する").font(.body).foregroundColor(.red).frame(maxWidth: .infinity) }
                        .padding(.top, 20).padding(.bottom, 40)
                    }
                }
                Button { saveChanges() } label: {
                    Text("保存する").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 56)
            ...
            .navigationTitle("器を編集").navigationBarTitleDisplayMode(.inline)
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button { dismiss() } label: { Image(systemName: "xmark") } } }
            .alert("入力エラー", isPresented: $showingValidationAlert) { Button("OK", role: .cancel) { } } message: { Text(validationMessage) }
            .alert("器を削除しますか？", isPresented: $showingDeleteConfirmation) {
                Button("キャンセル", role: .cancel) { }; Button("削除する", role: .destructive) { deleteContainer() }
...
struct RecordEditSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack(alignment: .bottom) {
                Color.backgroundGray.ignoresSafeArea()
                Form {
                    Section(header: Text("設置データ")) {
                        DatePicker("設置日時", selection: $tempStartTime)
                        HStack { Text("設置時重量"); Spacer(); TextField("g", text: $tempStartWeight).keyboardType(.numberPad).multilineTextAlignment(.trailing); Text("g") }
                    }
                    if isCollection {
                        Section(header: Text("回収データ")) {
                            DatePicker("回収日時", selection: $tempEndTime)
                            HStack { Text("回収時重量"); Spacer(); TextField("g", text: $tempEndWeight).keyboardType(.numberPad).multilineTextAlignment(.trailing); Text("g") }
                        }
                    }
                    Section { Button("この記録を削除", role: .destructive) { showingDeleteAlert = true } }
                }
                .padding(.bottom, 80)
                Button { saveChanges() } label: {
                    Text("保存する").fontWeight(.semibold).foregroundColor(.white).frame(maxWidth: .infinity).frame(height: 56).background(Color.appMain).cornerRadius(.buttonCornerRadius)
                }
            ...
            .alert("記録を削除しますか？", isPresented: $showingDeleteAlert) {
                Button("キャンセル", role: .cancel) {}; Button("削除", role: .destructive) { deleteRecord() }
            } message: { Text("この操作は取り消せません。") }
        }
    }
    ...
    private func deleteRecord() { modelContext.delete(record); try? modelContext.save(); dismiss() }
}
...
struct ContainerHistoryList: View {
    ...
    private var groupedItems: [String: [TimelineItem]] { Dictionary(grouping: timelineItems) { item in historyViewModel.formatDate(item.date) } }
    private var sortedDates: [String] {
        groupedItems.keys.sorted { d1, d2 in (DateFormatter.japaneseDate.date(from: d1) ?? Date()) > (DateFormatter.japaneseDate.date(from: d2) ?? Date()) }
    }
    ...
    var body: some View {
        ZStack {
            ...
            if timelineItems.isEmpty {
                VStack(spacing: 12) { Image(systemName: "clock").font(.largeTitle).foregroundColor(.secondary); Text("履歴はありません").font(.subheadline).foregroundColor(.secondary) }
                    ...
                    LazyVStack(spacing: 24) {
                        ForEach(sortedDates, id: \.self) { date in
                            VStack(alignment: .leading, spacing: 8) {
                                Text(date).font(.subheadline).fontWeight(.bold).foregroundColor(.secondary).padding(.leading, 4)
                                VStack(spacing: 12) {
                                    ForEach(groupedItems[date] ?? []) { item in
                                        Button {
                                            let id = item.recordID
                                            if let record = modelContext.model(for: id) as? WaterRecord { onTapRecord(record) }
                                        } label: {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(item.type == .setup ? (isFirst ? "設置中" : "設置") : "回収").font(.body).fontWeight(isFirst ? .bold : .regular)
